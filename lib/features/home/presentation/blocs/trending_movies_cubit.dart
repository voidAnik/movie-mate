...
class TrendingMoviesCubit extends CommonApiCubit<List<MovieModel>> {
  ...
  Future<void> fetchMovies({int page = 1}) async {
    ...
    //emit(ApiLoading());
    final Either<Failure, List<MovieModel>> failureOrResponse = await getTrendingMovies(params: page);
    ...
    failureOrResponse.fold(
          ...
          emit(ApiError(message: _mapFailureToMessage(failure)));
            ...
            (response) {
              existingMovies.addAll(response);
          if (page == 1) {
            // Reset the list if it's the first page
            emit(ApiSuccess(response: response));
          } else {
            // Append new movies to the existing list
            emit(ApiSuccess(response: List.from(existingMovies)));
          }
